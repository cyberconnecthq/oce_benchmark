...
def get_balances():
    ...
    receiver_add = "0xD117Bd6dE83e3F14265a3CE2BEEE6bd69d29eC7E"
    wens_balance = wrapper.functions.balanceOf(Web3.to_checksum_address(target_add), TOKEN_ID).call()
    wens_balance_receiver = wrapper.functions.balanceOf(Web3.to_checksum_address(receiver_add), TOKEN_ID).call()
    ...
    return (
        ...
        f"Balance of address {receiver_add}:\n"
        f"wENS balance: {wens_balance_receiver} wENS\n\n"
        f"{expire_info}"
    )
