...
# 4) 查询 wstETH 余额变化
# ————————————————————————————————————————————————————————————————
bal_after = wsteth.functions.balanceOf(OWNER).call()
delta     = (bal_after - bal_before) / 10**decimals
print(f"wstETH after  = {bal_after / 10**decimals:.6f}")
print(f"≈ minted      = {delta:.6f} wstETH")

# ————————————————————————————————————————————————————————————————
# 5) 简单 sanity check：wstETH ↔ stETH 汇率
# ————————————————————————————————————————————————————————————————
# 可选：读取 stEthPerToken() 或 tokensPerStEth() 进一步验证